...
def get_page_soup_from_url() -> list:
    ...
    for number in range(1, number_page + 1):
        base_doors = f'https://bunkerdoors.ru/prod/bunker-hit?page={number}'
        try:
...
def parsing_links_hit_doors():
    ...
    for file in files:
        page_soup = get_page_soup_from_file(file)

        receive_links = get_data_from_link(page_soup)
        print(receive_links)
